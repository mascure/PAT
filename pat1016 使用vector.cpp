...
struct Record
{
	string user,time,state;
}r;
vector<Record> all_record;
int charge[24];
//按姓名排序
bool cmp(const Record& a,const Record& b)
{
	if(a.user<b.user)return true;
	else if(a.user==b.user&&a.time<b.time)return true;
	else
	return false;
}
...
int main()
{
	...
	{
		getchar();
		cin>>r.user>>r.time>>r.state;
		all_record.push_back(r);
	}
	sort(all_record.begin(),all_record.end(),cmp);
